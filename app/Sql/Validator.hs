...
validateSqlSelect (Select cols from joins mWhere) =
  let
    ...
    mentionedTables = nub $
      ...
      [ toLowerStr t | (_, _, RightPredColumn rcol) <- fromMaybe [] mWhere ++ map snd joins
                     , let (t, _) = splitAtDot rcol, not (null t) ]
  ...
  in
    ((["Missing tables: " <> show missingTables | not (null missingTables)]) ++ (["Missing columns: " <> show missingColumns | not (null missingColumns)]))
